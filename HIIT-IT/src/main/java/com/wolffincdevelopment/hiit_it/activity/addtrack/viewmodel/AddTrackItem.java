...
public class AddTrackItem extends BaseViewModel implements TextWatcher, PreviewMusicService.PreviewListener {
    ...
    @Bindable
    public Drawable getPreviewButtonImage() {
        return previewIsPlaying ? VectorDrawableCompat.create(context.getResources(), R.drawable.ic_stop_dark_orange, null) :
                VectorDrawableCompat.create(context.getResources(), R.drawable.ic_play_circle_filled_deep_orange_48px, null);
    }

    public void setPreviewIsPlaying(boolean previewIsPlaying) {
        this.previewIsPlaying = previewIsPlaying;
        notifyPropertyChanged(BR.previewButtonImage);
    }
    ...
    public void onPreviewButtonClicked() {
        ...
        if (hasViewCallback()) {
            getViewCallback().onPreviewClicked(previewIsPlaying);
        }
    }
    ...
    private void checkFields() {
        ...
        if (binding.startTime.getText().toString().isEmpty() || binding.stopTime.getText().toString().isEmpty()
                || !binding.startTime.getText().toString().matches("^([0-9]{2}):([0-9]{2})$")
